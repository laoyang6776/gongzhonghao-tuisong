...
Options:
  ...
  --provider google|openai|openrouter|dashscope|replicate|jimeng|seedream|siliconflow  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
Environment variables:
  ...
  SILICONFLOW_IMAGE_MODEL   Default SiliconFlow model (Kwai-Kolors/Kolors)
  SILICONFLOW_BASE_URL      Custom SiliconFlow endpoint (default: https://api.siliconflow.cn/v1)
...
export function getConfiguredProviderRateLimits(
  ...
  const configured: Record<Provider, ProviderRateLimit> = {
    ...
    siliconflow: { ...DEFAULT_PROVIDER_RATE_LIMITS.siliconflow },
  };

  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "jimeng", "seedream", "siliconflow"] as Provider[]) {
...
export function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, ARK_API_KEY, or SILICONFLOW_API_KEY.\n" +
